...
export async function runPipeline(projectPath, options = {}) {
  ...
  cleanupOnExit()
  const execOptions = { cwd, variable, variablesFile, silent }
  if (options.shellExecutorNoImage !== undefined) {
    execOptions.shellExecutorNoImage = options.shellExecutorNoImage
  }
  const jobs = await execPipeline(execOptions)
